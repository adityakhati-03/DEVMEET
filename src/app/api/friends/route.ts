...
export async function GET() {
  try {
    ...
    const friends = (accepted as unknown as Array<{ _id: string, requester: { _id: { toString: () => string } }, recipient: unknown, updatedAt: string }>).map(f => ({
      friendshipId: f._id,
...
export async function POST(req: NextRequest) {
  try {
    ...
    if ((e as { code?: number }).code === 11000) return NextResponse.json({ message: 'Request already sent.' }, { status: 409 });
    console.error(e);
    return NextResponse.json({ message: 'Server error' }, { status: 500 });
  }
}
