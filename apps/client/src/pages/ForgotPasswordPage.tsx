...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.username) {
        setTimeout(() => navigate(`/reset-password?username=${result.username}`), 2000);
      }
    } catch (err: unknown) {
      ...
      <div className="flex w-full max-w-sm flex-col gap-6">
        <div style={{ background: 'var(--dm-card)', border: '1px solid var(--dm-border)', borderRadius: '12px', padding: '32px 24px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '28px' }}>
            ...
            <h1 style={{ fontSize: '24px', fontWeight: 800, color: 'var(--dm-text)', margin: 0 }}>Forgot Password</h1>
            <p style={{ color: 'var(--dm-muted)', fontSize: '14px', margin: '6px 0 0', textAlign: 'center' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            <div>
              ...
              <input
                type="email" placeholder="you@example.com" required disabled={loading}
            ...
            {message && <div style={{ padding: '10px 14px', background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.2)', borderRadius: '8px', color: '#34d399', fontSize: '13px' }}>{message}</div>}

            <button type="submit" disabled={loading}
              style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '14px', borderRadius: '8px', background: '#34d399', color: '#080a0f', fontWeight: 800, fontSize: '15px', border: 'none', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.7 : 1 }}>
              {loading ? <><Loader2 style={{ width: '16px', height: '16px' }} className="animate-spin" /> Sending...</> : 'Send Reset Code'}
            </button>
          </form>
        </div>
      </div>
