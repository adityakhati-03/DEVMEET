...
export default function SubmissionResultPanel({ submission, onClose }: SubmissionResultPanelProps) {
  ...
  return (
    <div style={{ position: 'fixed', bottom: '24px', right: '24px', width: '400px', maxHeight: '60vh', background: '#1e293b', border: '1px solid #334155', borderRadius: '12px', display: 'flex', flexDirection: 'column', boxShadow: '0 10px 25px -5px rgba(0,0,0,0.5)', zIndex: 100 }}>
      <div style={{ padding: '16px', borderBottom: '1px solid #334155', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 600, color: 'white', display: 'flex', alignItems: 'center', gap: '8px' }}>
      ...
      <div style={{ padding: '16px', overflowY: 'auto', flex: 1 }}>
        ...
        {!isRunning && submission.visibleResults && submission.visibleResults.length > 0 && (
          <div>
            <h4 style={{ margin: '0 0 8px', fontSize: '14px', color: '#9ca3af' }}>Visible Test Cases</h4>
            {submission.visibleResults.map((tc, idx) => (
              <div key={idx} style={{ marginBottom: '12px', padding: '12px', background: tc.passed ? 'rgba(52,211,153,0.05)' : 'rgba(248,113,113,0.05)', border: `1px solid ${tc.passed ? 'rgba(52,211,153,0.2)' : 'rgba(248,113,113,0.2)'}`, borderRadius: '8px' }}>
                <div style={{ fontSize: '13px', fontWeight: 600, color: tc.passed ? '#34d399' : '#f87171', marginBottom: '8px' }}>
        ...
        {!isRunning && submission.hiddenSummary && submission.hiddenSummary.total > 0 && (
          <div style={{ padding: '12px', background: 'rgba(255,255,255,0.02)', borderRadius: '8px', border: '1px solid #334155' }}>
            <h4 style={{ margin: '0 0 8px', fontSize: '14px', color: '#9ca3af' }}>Hidden Test Cases</h4>
